...
//        windowFactory.updateResult("正在准备生成项目文档...\n", project);
        ...
        String docFile = docDir + File.separator + "API文档_" + timestamp + ".md";

        // 生成项目文档
        AtomicReference<StringBuilder> content = new AtomicReference<>(new StringBuilder());

        String promt = analysisService.generateApiDocumentation(projectFiles);
        System.out.println("生成项目文档的promt:"+promt);

        // 在后台线程中执行文档生成
        ProgressManager.getInstance().run(
            new Task.Backgroundable(project, "生成项目api文档", true) {
                private volatile boolean isDisposed = false;

                @Override
                public void run(@NotNull ProgressIndicator indicator) {
                    indicator.setIndeterminate(true);
                    indicator.setText("正在生成项目api文档...");
